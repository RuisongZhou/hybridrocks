...
//   virtual void InsertNewTuningPoints(ChangePoint point) = 0;

 protected:
//   virtual void DetectAndTuning(int secs_elapsed) = 0;
  virtual Status ReportLine(int secs_elapsed, int total_ops_done_snapshot) = 0;
  Env* env_;
  ...
  void SleepAndReport() {
    ...
    while (true) {
      ...
      auto secs_elapsed =
          ...
          kMicrosInSecond;
    //   DetectAndTuning((int)secs_elapsed);
      auto s = this->ReportLine((int)secs_elapsed, (int)total_ops_done_snapshot);
...
//  public:
//   ReporterAgentWithSILK(DBImpl* running_db, Env* env, const std::string&
//   fname,
//                         uint64_t report_interval_secs, int32_t
//                         FLAGS_value_size, int32_t bandwidth_limitation);
...
class ReporterWithMoreDetails : public ReporterAgent {
 ...
 public:
  ...
  ~ReporterWithMoreDetails() = default;
//   void DetectAndTuning(int secs_elapsed) override {};
