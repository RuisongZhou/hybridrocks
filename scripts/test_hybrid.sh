...
sync
sudo bash -c "echo 3 > /proc/sys/vm/drop_caches" 
../db_bench --db=$dbname --db_path="$ssd_path:$ssd_size,$hdd_path:$hdd_size" --num=$num --reads=100000 --use_existing_db=1  --use_existing_keys=1 --compression_type=none --compression_ratio=1 \
